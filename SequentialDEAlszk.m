function [effsdea,zksdea] = SequentialDEAlszk(Xk,Yk,X,Y,ori,rts,prdl,Company,District,Companyk,Districtk)

% ------------------------
% Function for sequential DEA decentralized for lambda^s and z^k - code by Z. Wang (2023)
...
% Currently for Primal and Output-orientation only:
		for i = 1:Kk
            xktemp=[];
            ...
            % Extended ref X/Y
            XStemp = zeros(Kk*s,pp*s);
            YStemp = zeros(Kk*s,qq*s);
            for j = 1:length(Kinsub)
                ...
                XStemp(Kk*(j-1)+1:Kk*j, pp*(j-1)+1:pp*j) = XSt;
                YStemp(Kk*(j-1)+1:Kk*j, qq*(j-1)+1:qq*j) = YSt;
            end
            ...
            % Further extending the matrices from SequentialDEAls with S zk
            [n,p] = size(XStemp);
            [~,q] = size(YStemp);
            aq=[];
            bq=[];
             Skk = length(yktemp);
             f=[repmat(-1,Skk,1);zeros(Skk*Kk,1)];
            
            if strcmp('VRS',rts)

            Atemp = zeros(Skk,Kk); 
            for j = 1:Skk
                Atemp(j,Kk*(j-1)+1:Kk*j) = 1;
            end
             A=[repmat(0,Skk,Skk),Atemp;repmat(0,p,Skk),XStemp';diag(yktemp),-YStemp'];
             b=[ones(Skk,1);xktemp;zeros(q,1)];               
